...
rks = ["HEFT", "LB", "W"]
colors = {"HEFT" : '#988ED5', "LB" : '#E24A33', "W" : '#348ABD'}
...
for i, nw in enumerate(n_workers):
    ...
    apds = {rk : [0.0, 0.0, 0.0] for rk in rks}
    for h, m in [(1.0, "R"), (1.0, "UR"), (2.0, "R"), (2.0, "UR")]:
        for k, ccr in enumerate(ccrs):
            ndags = len(makespans[nw][ccr][h][m]["HEFT"])
            for j in range(ndags):
                best = min(makespans[nw][ccr][h][m][rk][j] for rk in rks) 
                for rk in rks:
                    mkspan = makespans[nw][ccr][h][m][rk][j]
                    pd = 25 - best/mkspan * 25
                    apds[rk][k] += (pd/ndags)  
    
    j = 0
    for rk in rks:
        ax1.bar(x + j * width, apds[rk], width, color=colors[rk], edgecolor='white', label=rk)
        j += 1   

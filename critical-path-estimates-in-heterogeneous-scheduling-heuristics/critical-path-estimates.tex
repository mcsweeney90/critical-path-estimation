...
\begin{itemize}
	\item $n \in \{ 100, 500, 1000 \}$, the number of non-entry/exit tasks in each of the graphs. Each graph also has a single entry and exit task so that altogether it has e.g., $102$ tasks rather than $100$. Once this parameter has been specified, we use the topologies of the corresponding set of that size from the STG. 
...
First we want to see how well HEFT actually does compared to a random sort...

Comments: difference between related and unrelated methods of generating costs (why? closer to MSP? Or something deeper?), failures, any differences in magnitude of improvement over random sort (e.g., size).

\subsection{Evaluation of proposed rankings}
\label{subsect.evaluation}

We compare the following alternative ranking phases....
\begin{itemize}
	\item Normal HEFT.
	\item Different tiebreaking criterion.
	\item Fulkerson.
	\item Weighted mean.
	\item Fulkerson-weighted mean.
	\item Lower bound.
\end{itemize}
%different tiebreaking criterion

Only considered Fulkerson for the smaller DAG sizes since two expensive---conclusion: doesn't appear to do any better, even the weighted version. But maybe discuss example for CPU-GPU. Monte Carlo results, same thing. 

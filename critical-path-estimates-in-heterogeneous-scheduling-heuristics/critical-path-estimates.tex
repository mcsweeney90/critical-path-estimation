...
\end{align} 
Note that this does not necessarily respect precedence constraints, so need additional step...
...
All of the methods for estimating critical path lengths at the prioritization phase which were introduced in the previous section can be modified to give conditional critical path estimates (which also disregard the cost of the task itself). The most straightforward is the optimistic lower bound, for which the conditional critical path lengths are given by $C_i^a = 0$ for all exit tasks and   
\begin{align}
C_i^a &= \max_{k \in S_i} \bigg( \min_{b = 1, \dots, q} \big( C_k^b + W_k^b + W_{ik}^{ab} \big)  \bigg), \quad a = 1, \dots, q, \label{eq.opt_lookahead} 
\end{align}
for all other tasks. Note that this is extremely similar to the optimistic costs \eqref{eq.peft_lookahead} as used in PEFT, with the exception that the specific communication cost $W_{ik}^{ab}$ is used in the minimization rather than the average $\overline{w_{ik}}$. Arguably this is the most intuitive way to define the conditional critical path since the value $C_i^a$ is a true lower bound on the remaining makespan of any schedule which executes task $t_i$ on processor $p_a$; locally-optimal processor selections are overruled if the best possible final makespan we can hope to achieve given that selection is inferior to other choices.          

Alternatively, we could take a similar tack to the standard HEFT upward ranks and use an estimate of what we expect the conditional critical paths to be. More specifically, let $C_i^a = 0$, for all exit tasks and $a = 1, \dots, q$, then move up the DAG and recursively compute 
\begin{align}
C_i^a = \max_{k \in S_i} \bigg( \frac{1}{q} \sum_{b = 1}^{q} (C_k^b + W_k^b + W_{ik}^{ab}) \bigg), \quad a = 1, \dots, q, \label{eq.uia} 
\end{align} 
for all other tasks. Note that we could just as easily use a weighted average inside the maximization, which is particularly attractive since the corresponding ranking performed well in the previous section. In particular, let $C_i^a = 0$, for all exit tasks and $a = 1, \dots, q$ as ever, and define
\begin{align}
...
All of these conditional critical path definitions can be used with any kind of averaging in order to compute task priorities.... We investigated several different choices for each empirically. However, given the superior performance of the weighted average task ranking that we have observed in our experiments so far, we decided that a ranking along those lines was the most sensible default choice. In particular, for all three different methods of defining the conditional critical path lengths $C_i^a$ given by equations \eqref{eq.opt_lookahead}--\eqref{eq.uia_hat}, we first compute for all $i = 1, \dots, n$,  
\begin{align*}
\hat{s}_i = \sum_{a} \frac{1}{W_i^a + C_i^a} 
\end{align*}    
and then define the corresponding task priorities by
\begin{align}
C_i = \frac{q}{\hat{s}_i}. \label{eq.chat_prios}
\end{align}  

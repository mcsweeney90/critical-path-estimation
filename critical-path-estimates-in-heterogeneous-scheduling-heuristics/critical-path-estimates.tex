...
While it may be worthwhile to investigate in future if the performance of the F and WF rankings improves for larger DAGs, given that the results so far suggest they offer no advantage compared to much cheaper alternatives, we decided to omit those two rankings when we repeated our comparison for sets of larger DAGs ($n = 1000$) from the STG. The metric we choose to use now us the APD... Figure X shows the APD for these graphs... Again we see that the W ranking does slightly better...

To summarize, our main conclusions from the experiments described in this section are as follows.
\begin{itemize}
	\item There appears to be no benefit from obtaining tighter bounds on the associated stochastic graph.
	\item There does appear to be slight improvement on average by using the weighted mean ranking W, although the effect is small and significant variation is seen for individual graphs.
	\item No apparent benefit to using the optimistic lower bound.
	\item Supporting RS study, the variation is great and it's difficult to say what the best approach is... 
\end{itemize} 

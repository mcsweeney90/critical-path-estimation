...
In this section we use our software simulator to evaluate the alternative task prioritization phases in HEFT described in Sections \ref{sect.optimistic} and \ref{sect.alt_rankings}, as well as the modification to PEFT outlined in Section \ref{sect.processor_selection}. All of the data from the experiments described here can be found in the Github repository associated with this chapter in the {\tt results}\footnote{\href{https://github.com/mcsweeney90/critical-path-estimation/results}{{\tt \small critical-path-estimation/results}}} directory. Furthermore, the directory also contains the scripts used to generate the data, which were executed on a machine with an Intel i9-9820X @3.30 GHz processor, running Ubuntu 18.04 and {\tt Python 3.6.9}. 
...
This approach to generating task computation costs differs from that used in the original HEFT \cite{topcuoglu2002performance} and PEFT \cite{arabnejad14} papers, which may help to explain any differences with results reported there, although most of the experiments described in this section were also repeated for the method they used, with substantially similar results.

Unlike the computation costs, communication costs are generated independently of the processor power. After all of the computation costs have been determined, we compute an average edge cost $\overline{e}$ such that the specified CCR will be achieved. Then for all edges $(t_i, t_k)$, we realize $\overline{w_{ik}}$ from a $U[0, 2\overline{e}]$ distribution. Specific communication costs between the tasks on all possible pairs of distinct processors are in turn selected uniformly at random from the interval $\big[ \overline{w_{ik}} (1 - h/2), \; \overline{w_{ik}} (1 + h/2)   \big]$.
...
\begin{figure}
	\centering	
	\subfloat[SLR.]{\label{plot.bench_slr}\includegraphics[scale=0.5]{slr_by_q.png}}
	\subfloat[Speedup.]{\label{plot.bench_speedup}\includegraphics[scale=0.5]{speedup_by_q.png}}
...
While we largely avoid discussion of comparative runtimes here because our code is not optimized, we should note that our implementation of PEFT could be significantly more expensive than HEFT, especially for larger values of $n$ and $q$. Note also that we do not consider the comparative {\em robustness} of the schedules computed by the two heuristics, or our proposed modifications of them, here since this topic is discussed in depth in a later chapter; at any rate, we did not see significant differences in this regard between the two.  
% Add something to end.

Given what we have observed in these benchmarking experiments, we decided to investigate the proposed alternative HEFT task ranking phases and PEFT variants separately, which we do in the following two sections, respectively. We then make direct comparisons and draw overall conclusions in Section \ref{subsect.conclusions}.
% TODO: should this be a conclusions subsection?
...
\begin{itemize}
	\item U, the standard upward ranks $u_i$,
...
Given the superior performance of the W ranking in the previous section, the three PEFT variants we consider here all use the task prioritization given by \eqref{eq.alt_prios} and are therefore defined by how they compute the conditional critical paths:
\begin{itemize}
	\item PEFT-LB, using \eqref{eq.cia_min}.
	\item PEFT-M, using \eqref{eq.cia_mean}.
	\item PEFT-WM, using \eqref{eq.cia_weighted}.
        \end{itemize}
        ...
        With regards to the former, Figure \ref{plot.better_by_q} illustrates the percentage of instances across the DAG test set for which each of the three variants achieved a superior schedule to the standard PEFT algorithm, broken down by processor number $q$. Overall PEFT-LB is most likely to improve on the default, obtaining a better schedule $55\%$ of the time across the entire set, with PEFT-WM second at $53\%$. One of the questions prompted by the benchmarking experiments in Section \ref{subsect.benchmarking} was whether any of these variants could reduce the number of failures, but in fact all of them had a higher failure rate. PEFT-LB's was only slightly higher but the M and WM variants both recorded almost double the number of failures---this equates to about $5\%$ of the entire DAG set, concentrated largely in the same parameter regimes identified earlier (i.e, $\beta = 0.1$ and $h = 2.0$). Furthermore, when they failed, they could do so extremely badly, which is why we chose to display the percentage of better instances in the figure rather than the APD: sometimes the makespan could be literally thousands of times worse, leading to distorted APD values.

        \begin{figure}
	\centering	
	\includegraphics[scale=1.0]{better_by_q.png}
	\caption{Percentage of DAGs for which the three PEFT variants achieved a better schedule than the standard heuristic, by number of processors $q$.}	
	\label{plot.better_by_q}
      \end{figure}

      As to whether the processor selection phase was effective, results were mixed. For the M and WM variants, the processor selection phase did not appear to be beneficial compared to just using the task prioritization phase defined by \eqref{eq.alt_prios} with the HEFT processor selection. For both variants, the percentage of DAGs for which the processor selection led to a better schedule was only about $40\%$. Furthermore, while it did not substantially increase the number of failures in either case, the processor selection phase often made the resulting schedule much worse, sometimes by orders of magnitude. Results were much more positive for the LB variant, however: there was a $1.3\%$ reduction in schedule length compared to the ranking-only version and the number of failures was also substantially reduced, suggesting that the processor selection phase was effective.     
...
\begin{itemize}
	\item Largely supporting previous investigations along these lines \cite{zhao03}, perhaps our biggest takeaway is that no method appears to be dominant and it is extremely difficult to say, for a given DAG and target platform, which should be used. 
	...
	\item Similarly, there appears to be little advantage in using the LB ranking, although it also wasn't clearly inferior to the standard ranking in terms of performance or cost.  
        \end{itemize}
        With regards to the modified PEFT heuristics, we make the following conclusions.
        \begin{itemize}
        \item PEFT-M and PEFT-WM do not appear to offer any advantage compared to the standard heuristic and indeed can display adverse behavior in some situations, in particular when communication predominates.
          \item PEFT-LB did however perform well relative to PEFT and both the alternative task prioritization and processor selection phases appeared to be beneficial, at least on average.
          \end{itemize}
